...
function change_cart_page_url() {
  if (typeof taa.home_permalink !== "undefined" && taa.home_permalink != "") {
    ...
    jQuery(".elementor-button--checkout, .checkout-button").each(function (
      index
    ) {
...
jQuery(document).ready(function () {
  ...
  if (window.self !== window.top) {
    jQuery(document)
      .find(
        ".tbdemo-sharebar-container,.tbdemo-ai-popup-layout,.tbdemo-ai-popup-container,.tbdemo-customize-container"
      )
      .remove();
  }
  ...
  jQuery(".tbdemo-circle-icon-content .tbdemo-share-button")
    .parent()
    .on("click", function () {
      jQuery(".tbdemo-big-popup, .tbdemo-big-popup-overlay").show();
    });
  jQuery(
    ".tbdemo-big-popup-close, .tbdemo-big-popup-overlay, .tbdemo-button-promo-got-it"
  ).on("click", function () {
  ...
  if (guest) {
    ...
    if (taa.is_home == 1) {
      ...
      jQuery(window).on("scroll", function () {
        ...
        if (
          jQuery(".tbdemo-editor-cont").length !== 1 &&
          !displayed_1wf &&
          displayed_1wf === "" &&
          tbdemo_amountscrolled() > 70
        ) {
  ...
  jQuery(
    ".tf-v1-popover-button:not(.open .tf-v1-popover-button), .tbdemo-circle-icon:not(.tbdemo-close-circle)"
  ).hover(
    function (e) {
      if (
        jQuery(".tf-v1-popover.open").length ||
        (jQuery(".tbdemo-customize-container").length &&
          jQuery(e.target).hasClass("tbdemo-customize"))
      ) {
        return;
      }
      let cont = jQuery("<div>", {
        ...
        html: jQuery(this)
          .closest(".tbdemo-circle-icon-border")
          .data("tooltip"),
      });
      ...
      if (
        jQuery(".tbdemo-sharebar-container").hasClass("tbdemo-customize-open")
      ) {
  ...
  jQuery(".twbb-pu-upgrade-layout, .twbb-pu-upgrade-close").on(
    "click",
    function () {
  ...
  jQuery(document)
    .find(".tbdemo-popup-share-button")
    .on({
      mouseenter: function () {
...
function tbdemo_show_editor() {
  ...
  let add_page_link =
    ...
    "/ai-builder?add_page=1";
  let add_dashboard_link =
    twbb.tenweb_dashboard +
    "/websites/" +
    twbb.dashboard_website_id +
    "/ai-builder?from_demo=1";
  ...
  jQuery(".tbdemo-editor-top-bar").append(
    '<div class="tbdemo-btn-container"></div>'
  );
...
function tbdemo_animate(open) {
  ...
  if (
    jQuery(
      "div[data-elementor-type='twbb_header'] .elementor-section-wrap > .elementor-element.elementor-sticky--active"
    ).length
  ) {
    sticky_active = true;
  }
  ...
  jQuery(
    "div[data-elementor-type='twbb_header'] .elementor-section-wrap > .elementor-element"
  )
    .css({
      top: open ? 0 : "48px",
    })
    .animate(
      {
        ...
        width: open
          ? jQuery(window).width() - 222 + "px"
          : jQuery(window).width() + "px",
        left: open ? "unset" : "auto",
      },
      "slow"
    );
  jQuery(".tbdemo-editor-cont, .tbdemo-editor-top")
    .css({
      left: open ? "-222px" : 0,
    })
    .animate(
  ...
  jQuery("body.tbdemo-editor-mode")
    .css({
      left: open ? 0 : "222px",
      top: open ? 0 : "48px",
      width: open ? "100%" : calc,
  ...
  if (jQuery("#tbdemo-device-iframe:visible").length) {
    jQuery(document)
      .find(".tbdemo-device-laptop")
      .removeClass("tbdemo-device-active");
    jQuery(document)
      .find(".tbdemo-device-mobile")
      .addClass("tbdemo-device-active");
    let left = open
      ? jQuery(window).width() / 2 + 111 - 180
      : jQuery(window).width() / 2 - 180;

    let buttons_container = jQuery("body .tbdemo-devicebar-border");
    let height_px =
      parseInt(buttons_container.css("bottom")) +
      parseInt(buttons_container.css("height")) +
      80;
    ...
    jQuery("#tbdemo-device-iframe:visible").animate(
      {
        ...
        height: open
          ...
          : jQuery(window).height() - height_px,
      },
      500
    );
  } else {
...
function tbdemo_hide_editor_complete() {
  ...
  jQuery(
    ".tbdemo_selected_area, .tbdemo_container_edit_settings, .tbdemo_eicon"
  ).remove();
...
function tbdemo_fake_editor_marker() {
  ...
  let container_edit_icons =
    "<div class='tbdemo_container_edit tbdemo_selected_area'></div><ul class='tbdemo_eicon tbdemo_container_edit_settings'>";
  container_edit_icons +=
    "<li class='tbdemo_container_edit_settings-add' title='Add Container'>";
  container_edit_icons += "<i class='eicon-plus' aria-hidden='true'></i>";
  container_edit_icons += "</li>";
  container_edit_icons +=
    "<li class='tbdemo_container_edit_settings-edit' title='Edit Container'>";
  container_edit_icons += "<i class='eicon-handle' aria-hidden='true'></i>";
  container_edit_icons += "</li>";
  container_edit_icons +=
    "<li class='tbdemo_container_edit_settings-remove' title='Delete Container'>";
  container_edit_icons += "<i class='eicon-close' aria-hidden='true'></i>";
  container_edit_icons += "</li>";
  container_edit_icons += "</ul>";

  if (window.self === window.top) {
    jQuery(".elementor-section-wrap > [data-element_type='container']").each(
      function () {
        jQuery(this).append(container_edit_icons);
        jQuery(this)
          .find("div[data-element_type='container']")
          .append(
            "<div class='tbdemo_column_edit tbdemo_selected_area'></div><i class='tbdemo_eicon tbdemo_eicon-handle eicon-handle'></i>"
          );
        ...
        widget.each(function () {
          let e = jQuery(this);
          e.append(
            "<div class='tbdemo_widget_edit tbdemo_selected_area'></div><i class='tbdemo_eicon tbdemo_eicon-edit eicon-edit'></i>"
          );
  ...
  jQuery(iframe)
    .find(".elementor-section-wrap > [data-element_type='container']")
    .each(function () {
      jQuery(this).append(container_edit_icons);
      jQuery(this)
        .find("div[data-element_type='container']")
      ...
      jQuery(this)
        .find("div[data-element_type='widget']")
        .append(
          "<div class='tbdemo_widget_edit tbdemo_selected_area'></div><i class='tbdemo_eicon tbdemo_eicon-edit eicon-edit'></i>"
        );
      let widget = jQuery(this).find("div[data-element_type='widget']");
...
function tbdemo_hover_action(cont) {
  jQuery(cont)
    .find(".tbdemo_selected_area, .tbdemo_eicon, .tbdemo-ai-front")
    ...
    .on("mouseleave", function (e) {
      ...
      jQuery(cont)
        .find(
          ".tbdemo_selected_area, .tbdemo_eicon:not(.tbdemo_eicon-regenerate)"
        )
        .css("opacity", 0);
    });

  jQuery(cont)
    .find(".tbdemo_selected_area, .tbdemo_eicon")
    .on("click", function (e) {
      if (
        jQuery(e.target).hasClass("tbdemo-ai-front") ||
        jQuery(e.target).parents(".tbdemo-ai-front").length > 0
      ) {
        return;
      }
      if (jQuery(cont).find(".tbdemo-ai-front-opened").length) {
        jQuery(cont)
          .find(".tbdemo-ai-front-opened")
          .removeClass("tbdemo-ai-front-newpromp-open")
          .removeClass("tbdemo-ai-front-opened")
          .find(
            ".tbdemo-ai-front-button-layer, .tbdemo-ai-front-action-cont, .tbdemo-ai-front-new_prompt-container"
          )
          .hide();
        return;
      }
  ...
  jQuery(cont).on("click", ".tbdemo-ai-front-button", function () {
    jQuery(this)
      .closest(".tbdemo_selected_area")
      .addClass("tbdemo-ai-front-opened");
    tbdemo_show_ai_action_container(jQuery(this));
  });

  jQuery(cont).on("click", ".tbdemo-ai-front-action-button", function () {
    jQuery(cont)
      .find(".tbdemo-ai-front-opened, .tbdemo-ai-front-newpromp-open")
      .removeClass("tbdemo-ai-front-newpromp-open")
      .removeClass("tbdemo-ai-front-opened")
      .find(
        ".tbdemo-ai-front-button-layer, .tbdemo-ai-front-action-cont, .tbdemo-ai-front-new_prompt-container"
...
function tbdemo_show_new_propmt(that, cont) {
  ...
  jQuery(cont)
    .find(".tbdemo-ai-front-new_prompt-textarea")
    .on("keyup", function (e) {
      if (jQuery(this).val() == "") {
        jQuery(this)
          .parent()
          .find(".tbdemo-ai-front-new_prompt-action-button")
          .addClass("tbdemo-ai-front-button-disabled");
      } else {
        jQuery(this)
          .parent()
          .find(".tbdemo-ai-front-new_prompt-action-button")
          .removeClass("tbdemo-ai-front-button-disabled");
      }
    });
}
...
function tbdemo_show_ai_action_container(that) {
  ...
  jQuery(that)
    .closest(".tbdemo-ai-front")
    .find(".tbdemo-ai-front-layout, .tbdemo-ai-front-action-cont")
    .show();
}

function tbdemo_show_upgrade_popup(type) {
  jQuery(".twbb-pu-upgrade-layout, .twbb-pu-upgrade-container")
    .removeAttr("style")
    .removeClass("twbb-pu-hidden");
...
function tbdemo_showTypeform() {
  ...
  let displayed_typeform = tbdemo_getCookie(
    "tbdemo_submitted_typeform_" + taa.home_page_id
  );
  /* Remove typeform if  user submit once */
  if (
    (displayed_typeform && displayed_typeform !== "") ||
    jQuery(window).width() < 860 ||
    typeof window.tf == "undefined"
  ) {
    jQuery(".tbdemo-circle-icon-typeform-cont").remove();
    jQuery(".tbdemo-sharebar-container.tbdemo-typeform-active").removeClass(
      "tbdemo-typeform-active"
    );
    return;
  }

  jQuery(document)
    .find(".tbdemo-customize-container.tbdemo-customize-container-mobile")
    .css("bottom", "170px");
  ...
  let displayed_typeform_popup = tbdemo_getCookie(
    "tbdemo_displayed_typeform_" + taa.home_page_id
  );
  ...
  var formId = "T34BxPCi";
  window.tf.createPopover(formId, params);
}
...
function tbdemo_change_domain_id_url() {
  ...
  twbb_sidebar_vars.upgrade_url = str.replace(
    mathch1,
    mathch1.replace(mathch2, domain_id)
  );
}
