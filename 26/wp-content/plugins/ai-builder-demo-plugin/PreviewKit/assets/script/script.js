...
function tbdemo_customize_container(show) {
  if (show) {
    ...
    jQuery(document)
      .find(".tbdemo-customize-edit-button")
      .on("click", function () {
        jQuery(document)
          .find(".twbb-pu-upgrade-container")
          .addClass("tbdemo-from-customize");
        upgrade_popup_customize_title = jQuery(document)
          .find(".twbb-pu-upgrade-title")
          .text();
        jQuery(document)
          .find(".twbb-pu-upgrade-title")
          .text(taa.upgrade_popup_customize_title);
        upgrade_popup_customize_descr = jQuery(document)
          .find(".twbb-pu-upgrade-descr")
          .text();
        jQuery(document)
          .find(".twbb-pu-upgrade-descr")
          .text(taa.upgrade_popup_customize_descr);
...
function tbdemo_animate_customization(open) {
  ...
  jQuery(".tbdemo-customize-container")
    .css({
      right: open ? "-222px" : 0,
    })
    .animate(
  ...
  jQuery("body.tbdemo-customize-mode")
    .css({
      right: open ? 0 : "222px",
      width: open ? "100%" : calc,
    })
  ...
  jQuery(".tbdemo-sharebar-container")
    .css({
  ...
  jQuery(".tbdemo-circle-icon-border .tf-v1-popover-button")
    .css({
...
jQuery(document).ready(function () {
  jQuery(window).on("scroll", function () {
    if (jQuery("body").hasClass("tbdemo-editor-mode")) {
      ...
      jQuery(
        "div[data-elementor-type='twbb_header'] .elementor-section-wrap > .elementor-element:not(.elementor-sticky--active)"
      ).css({
        top: 0,
      });
    }
  ...
  jQuery(document)
    .find(".tbdemo-customize-close, .tbdemo-customize-container-overlay")
    .on("click", function () {
      tbdemo_customize_container(false);
    });
  ...
  jQuery(".twbb-pu-upgrade-layout, .twbb-pu-upgrade-close").on(
    ...
    function () {
      if (
        jQuery(document)
          .find(".twbb-pu-upgrade-container")
          .hasClass("tbdemo-from-customize")
      ) {
        jQuery(document)
          .find(".twbb-pu-upgrade-container")
          .removeClass("tbdemo-from-customize");
        jQuery(document)
          .find(".twbb-pu-upgrade-title")
          .text(upgrade_popup_customize_title);
        jQuery(document)
          .find(".twbb-pu-upgrade-descr")
          .text(upgrade_popup_customize_descr);
      }
    }
  );
  ...
  if (tbdemo_theme != "") {
    ...
    if (window.self === window.top) {
      jQuery(".tbdemo-theme-item").removeClass("tbdemo-theme-selected");
      jQuery(
        ".tbdemo-theme-item.tbdemo-" + tbdemo_theme.toLowerCase()
      ).addClass("tbdemo-theme-selected");
    }
  } else {
    /* Active theme in all_kit.json */
    jQuery(".tbdemo-theme-item").removeClass("tbdemo-theme-selected");
    jQuery(".tbdemo-theme-item.tbdemo-" + tbdemo.active_theme).addClass(
      "tbdemo-theme-selected"
    );
  ...
  for (const key in kitSettings) {
    if (key == "system_typography" || key == "custom_typography") {
      kitSettings[key].forEach(function (el) {
        let id = el["_id"];
        let font_size =
          el["typography_font_size"]["size"] +
          el["typography_font_size"]["unit"];
        jQuery("body").css(
          "--e-global-typography-" + id + "-font-family",
          el["typography_font_family"]
        );
